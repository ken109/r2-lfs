...
import { clearGithubCache, type Fetcher } from "../src/auth.ts";
...
describe("routing and configuration", () => {
  ...
  it("does not echo tokens from malformed AUTH_TOKENS entries", () => {
    expect(() => loadConfig(makeEnv({ AUTH_TOKENS: "acme/*:admin:supersecretvalue1234" }))).toThrow(
      /AUTH_TOKENS entry #1/,
    );
    try {
      loadConfig(makeEnv({ AUTH_TOKENS: "acme/*:admin:supersecretvalue1234" }));
    } catch (err) {
      expect(String(err)).not.toContain("supersecretvalue1234");
    }
  });

  it("requires credentials for presigned mode when forced", () => {
    expect(() => loadConfig(makeEnv({ TRANSFER_MODE: "presigned" }))).toThrow(/R2_ACCOUNT_ID/);
  });
...
describe("proxy transfers", () => {
  ...
  it("reports missing and malformed objects per object", async () => {
    const object = await blob();
    const { status, objects } = await batch(makeEnv(), "/acme/app", "download", [
      object,
      { oid: "not-an-oid", size: 1 },
    ]);
...
describe("presigned transfers", () => {
  ...
  it("does not cap upload size", async () => {
    ...
    const { objects } = await batch(presignEnv(), "/acme/app", "upload", [
      { oid: object.oid, size: 5 * 1024 * 1024 * 1024 },
    ]);
