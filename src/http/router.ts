...
const LFS_ROUTE = new RegExp(
  `^/(${OWNER_NAME})/(${REPO_NAME}?)(?:\\.git)?(?:/info/lfs)?/(objects/batch|objects/verify|objects/([0-9a-f]{64})|locks|locks/verify|locks/([A-Za-z0-9-]{1,64})/unlock)$`,
);
...
export function route(pathname: string): Route {
  ...
  const [, owner, name, endpoint, oid, lockId] = match as unknown as [
    string,
    string,
    string,
    string,
